...
This prerelease updates the original alpha release with stronger beta-testing
materials, clearer package documentation, deployed pkgdown pages, and
multi-platform check coverage.
...
Users who want the latest development build can install from `main`:
...
- local `R CMD build`;
- local `R CMD check --no-manual --as-cran`, with expected new-submission note
  and a local environment time-verification note;
- GitHub Actions R CMD check on Windows release, macOS release, Ubuntu release,
  and Ubuntu devel;
